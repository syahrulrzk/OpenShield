...
type Analysis = {
  ...
  dbTypeBreakdown: { dbType: string; status: string; count: number }[];
      ...
      <div className="flex items-end justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-[var(--muted-foreground)] mt-0.5">
            Trends, top attackers, dan patterns dari {data.period.days} hari terakhir
          </p>
        </div>
        <div className="flex items-center gap-2 text-xs">
          ...
          <select
            ...
            className="bg-[var(--background)] border border-[var(--border)] rounded-lg px-2.5 py-1 text-xs font-mono"
          >
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[500px]">
              <div className="grid grid-cols-[40px_repeat(24,1fr)] gap-0.5 text-[9px]">
                ...
                {heatmap.map((row, dow) => (
                  <>
                    <div key={`d-${dow}`} className="text-[var(--muted-foreground)] font-mono text-right pr-2">
                      {DOW_LABELS[dow]}
                    </div>
                    {row.map((count, hour) => (
                      <div
